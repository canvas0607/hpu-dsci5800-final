...
1. 部署路径：系统将部署在哪里？谁会使用它？它会是聊天机器人、网页应用、移动应用、仪表盘、API，还是内部工作流工具？
   ```aiignore
    部署路径见网址 http://192.209.63.103:8000/;部署在web服务器上采用了C/S架构 ，产品的用户使用pc/手机浏览器直接访问，他会是一个交互式的聊天模型，最终给用户一套家具方案
    ```
2. 评估指标：你将如何衡量成功？可选指标包括准确率、实用性、响应时间、用户满意度、安全通过率、幻觉率、任务完成率或多模态质量。
   ```aiignore
    首要衡量成功的指标是实用性和安全通过率，这两个在测试中通过。剩余的指标在于用户体验度，包括响应时间，任务完成率，和幻觉率，这边在完成率和幻觉率采用了checkpoint技术和外部API调用采集数据来减少
    相应时间依赖于服务器配置和使用的模型等级，这点如果是商业使用可以提高预算。
   ```
3. 人机协同设计：应在什么情况下由人类审核智能体的输出？哪些决策不应完全自动化？
    ```
    主要情况在于智能体需要首先确认用户需求，确定知识边界，不能过度的去讨论非相关问题。所以需要引导用户在知识范围内去下决策
    ```
4. 风险管理：可能会出现什么问题？你将如何在部署后监控系统？
    ```
    最大的问题在于攻击和模型恶意用户不断地使用，针对这两个问题，监控系统首先可以采用ELK来做日志收集，然后做数据分析提取恶意攻击日志，进行代码优化。第二需要使用Prometheus+grafana来对系统进行实时监控报警处理，设置各种监控指标。特别是api中的模型key消耗情况
   ```
5. 改进计划：应如何根据用户反馈或评估结果更新系统？
   ```aiignore
   根据用户留存度和收集请求日志，查看用户关心的热点数据，还有每次请求反馈的响应时间(P99指标)，来针对产品侧和服务器指标优化对整个产品进行提升
   ```